...
class Fill_Data_Table_With_Virtual_Servers():
    ...
    def ServerListData(self, ns_session):
        ...
        all_ssl_types = ['SSL', 'SSL_TCP', 'ANY', 'SIP_SSL', 'SSL_PUSH',
                         'SSL_DIAMETER', 'SSL_FIX', 'USER_SSL_TCP']

        # Fetches all the present LB_Virtual Servers
        result = lbvserver.get(ns_session)

        for i in (range(0, len(result))):
            # filter type to only show SSL
            if result[i].servicetype in all_ssl_types:
        ...
        # Fetches all the Content Switching servers
        ...
        # Fetches all the authentication servers (AAA)
...
class SSL_Hardening_Procedure():
    ...
    def Check_If_SSL_Profile_Present(self, ServerName, ns_session, Selected_Template, overwrite_profile_checkbox_status):
        ...
        try:
            ...
            # name of current applied profile
            profile_name = Current_SSL_Server.sslprofile

            # Check if a profile is present AND that it is created by Turbo_ADC
            # if true,  start evaluation against selected template
            if Current_SSL_Server.sslprofile is f"{ServerName}_Turbo_ADC_Profile":
                print("Updating Turbo Profile")
                return self.Update_SSL_Profile_And_Get_Values(ns_session, ServerName, profile_name)

            # If profile is not present or not made by Turbo_ADC create a new one
            elif Current_SSL_Server.sslprofile is not (f"{ServerName}_Turbo_ADC_Profile", "ns_default_ssl_profile_frontend", "ns_default_ssl_profile_secure_frontend") and None and overwrite_profile_checkbox_status == True:
                print("Updating custom profile")
                return self.Update_SSL_Profile_And_Get_Values(ns_session, ServerName, profile_name)

            # Create new profile
            else:
                print("Creating new profile")
                return self.Create_New_SSL_Profile(ns_session, ServerName)
    ...
    def Create_New_SSL_Profile(self, ns_session, ServerName):

        try:
            # Create new SSL Profile
            profile_creation = sslprofile()
            profile_name = f"{ServerName}_Turbo_ADC_Profile"
            profile_creation.name = profile_name

            profile_creation.add(
                ns_session, profile_creation)
        ...
        try:
            ...
            ssl_vserver_entry = sslvserver()
            ssl_vserver_entry.vservername = f"{ServerName}"
            ssl_vserver_entry.sslprofile = profile_name
            profile_name = ssl_vserver_entry.sslprofile
    ...
    def Update_SSL_Profile_And_Get_Values(self, ns_session, ServerName, profile_name):
        ...
        # Assign current settings to a dictionary
        ...
        # Start comparison
        return self.SSL_Profile_Compare_Settings(ns_session, current_ssl_profile_values, ServerName, profile_name)

    def SSL_Profile_Compare_Settings(self, ns_session, current_ssl_profile_values, ServerName, profile_name):
        try:
                ...
                for y in x:
                    if type(y) == (str):
                        ...
                        for current_set_value in y.values():
                            ...
                            break
            # Zip comparison list and return to UI.py
            comparison_list_zipped = zip(
                hostname, settings, old_value, new_value, profile_name_list)
            ...
            errormessage = ("SSL_Profile_Compare_Settings: " + str(error.args))
            print(errormessage)
...
class FixIssues():
    ...
    def FixSelectedIssues(self, ns_session, selected_issues_to_fix):
        for i in selected_issues_to_fix:
            ServerName, setting, new_value, profile_name = i[
                'hostname'], i['setting'], i['new_value'], i['profile_name']

            print(ServerName, setting, new_value, profile_name)
